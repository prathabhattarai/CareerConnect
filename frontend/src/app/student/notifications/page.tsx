...
export default function NotificationsPage() {
  ...
  if (authLoading || loading) return <div className="flex"><Sidebar role={(user?.role || 'student') as 'student' | 'company'} /><div className="flex-1 p-8"><div className="space-y-3">{Array.from({length:4}).map((_,i)=><div key={i} className="h-20 bg-surface-100 rounded-xl animate-pulse" />)}</div></div></div>;
  ...
  return (
    <div className="flex">
      <Sidebar role={role as 'student' | 'company'} />
